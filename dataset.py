...
class Dataset:
    def __init__(self, device='cpu', data_folder=None, transformed_data_folder=None):
    ...
    def transform(input_dataframe):
        ...
        for i in range(1, 11):
            ...
            df["SAME_DATE_SHIFT_" + str(i)] = (
                    (pd.to_datetime(df["DATE_SHIFT_" + str(i)])).dt.day == (pd.to_datetime(df['DATE'])).dt.day).astype(int)
            ...
            df["TOP_TAIL_SHIFT_" + str(i)] = (df["HIGH_SHIFT_" + str(i)] - df[["OPEN", "CLOSE"]].max(axis=1)) / (
                        df["HIGH_SHIFT_" + str(i)] - df["LOW_SHIFT_" + str(i)])
            df["BOT_TAIL_SHIFT_" + str(i)] = (df[["OPEN", "CLOSE"]].min(axis=1) - df["LOW_SHIFT_" + str(i)]) / (
                        df["HIGH_SHIFT_" + str(i)] - df["LOW_SHIFT_" + str(i)])
            df["BODY_SHIFT_" + str(i)] = (df["OPEN_SHIFT_" + str(i)] - df["CLOSE_SHIFT_" + str(i)]) / (
                        df["HIGH_SHIFT_" + str(i)] - df["LOW_SHIFT_" + str(i)])

            df["HIGH_DIV_LOW_SHIFT" + str(i)] = (df["HIGH_SHIFT_" + str(i)]) / (df["LOW_SHIFT_" + str(i)])
            df["OPEN_DIV_LOW_SHIFT" + str(i)] = (df["OPEN_SHIFT_" + str(i)]) / (df["LOW_SHIFT_" + str(i)])
            df["CLOSE_DIV_LOW_SHIFT" + str(i)] = (df["CLOSE_SHIFT_" + str(i)]) / (df["LOW_SHIFT_" + str(i)])
        ...
        for i in range(2, 42):
            df["OPEN_DIV_SHIFT_" + str(i)] = df['OPEN'].shift(1) / df['OPEN'].shift(i)
            df["CLOSE_DIV_SHIFT_" + str(i)] = df['CLOSE'].shift(1) / df['CLOSE'].shift(i)
            df["HIGH_DIV_SHIFT_" + str(i)] = df['HIGH'].shift(1) / df['HIGH'].shift(i)
            df["LOW_DIV_SHIFT_" + str(i)] = df['LOW'].shift(1) / df['LOW'].shift(i)

        for i in range(2, 84, 4):
            df["OPEN_DIV_WINDOW_SHIFT_" + str(i)] = df['OPEN'].shift(1) / open_avg.shift(i)
            df["CLOSE_DIV_WINDOW_SHIFT_" + str(i)] = df['CLOSE'].shift(1) / close_avg.shift(i)
            df["HIGH_DIV_WINDOW_SHIFT_" + str(i)] = df['HIGH'].shift(1) / high_avg.shift(i)
            df["LOW_DIV_WINDOW_SHIFT_" + str(i)] = df['LOW'].shift(1) / low_avg.shift(i)
    ...
    def transform_all_data(self):
        ...
        for root, dirs, files in os.walk(self.data_folder):
            ...
            for file in files:
                ...
                print(f"\rTransforming file {i}\tout of {n}", end='', flush=True)
                df = self.transform(df)
                df.to_csv(self.transformed_data_folder+"\\"+file, index=False)
                i+=1
    ...
    def split_files_train_val(self, train_part = 0.8):
        ...
        for root, dirs, files in os.walk(self.transformed_data_folder):
            for file in files:
                if ".csv" not in file:
                    continue
                size = os.path.getsize(self.transformed_data_folder+"\\" + file)
                total_size += size
        ...
        for i in range(len(files_and_sizes) , 0, -1):
            all_less = True
            print(f"\rTrying subset size {i}\tClosest = {closest_sum / total_size:.2f}", end='', flush=True)
            for subset in combinations(files_and_sizes, i):
                current_size = sum([el[1] for el in subset])
                if abs(target_train_size - current_size) < abs(target_train_size - closest_sum):
            ...
            if all_less:
                break
        if closest_sum == total_size or closest_sum == 0 or closest_sum == float('inf'):
            raise ValueError('something went wrong. try choosing train_files and val_files manually')
        self.train_files = []
        self.val_files = []
        ...
        print(f"\rBest split = {closest_sum/total_size:.2f}", end='', flush=True)
        #return best we can do
        return closest_sum/total_size

    def return_train_batches(self, batch_size=10_000):
        if self.train_files is None:
            raise ValueError('train_files not given. run self.split_files_train_val or fill manually')
        ...
        for file in shuffled_files:
            ...
            while (i+1)*batch_size <= n:
                yield torch_df[i*batch_size:(i+1)*batch_size].to(self.device), open_prices[i*batch_size:(i+1)*batch_size]
                i+=1
    def return_train_batches_count(self, batch_size=10_000):
        ...
        for file in self.train_files:
            ...
            print(f"\rCounting batches: file№ {i}\tout of {files_count}\t {i*100/files_count:.2f}%", end='', flush=True)
            total += row_count//batch_size
            i+= 1
        return total

    def return_val_batches(self):
        if self.val_files is None:
            raise ValueError('val_files not given. run self.split_files_train_val or fill manually')
    ...
    def return_val_batches_count(self):
        if self.val_files is None:
            raise ValueError('val_files not given. run self.split_files_train_val or fill manually')
