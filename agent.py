...
class Agent:
    def __init__(self, lr=0.001,long_memory_sampled = 1000, long_memory_size = 100000, gamma=0.9, device = 'cpu'):
    ...
    def get_state(self, row, cur_price, cur_money, cur_count, first_price):
        x = torch.tensor([cur_price / cur_money, (cur_price+cur_count)/(cur_price+cur_count), cur_price/first_price]).to(self.device)
    ...
    def train_short_memory(self, batch_size = 10_000):
        ...
        mini_sample = [self.memory[i] for i in range(len_memory+1-batch_size,len_memory)]
        states, actions, rewards, next_states= zip(*mini_sample)
